...
#-------------------------------------
TrainSet      = SleepDataset("/root/Desktop/Net/TRAIN")
print("Train set size",len(TrainSet))
TestSet       = SleepDataset("/root/Desktop/Net/TEST")
...
Model     = Sleep_Net_1().to(device)
#Model    = torch.load("/root/Desktop/Net/Sleep_Net_2.pt").to(device)
...
def tarin():
    ...
    for epoch in range(EPOCHS):
        ...
        #-----------------------------
        try:
            with open("/root/Desktop/Net/Record_Train.txt","a+") as File:
                File.write("%.3f\n"%(LOSS_SUM ))
        except:
            pass
        if (epoch) % 2 == 0: 
            ...
            with open("/root/Desktop/Net/Record_Test.txt","a+") as File:
                File.write("%.5f\n"%(Test_Result))
            torch.save(Model,"/root/Desktop/Net/Sleep_Net_2.pt")
        #-----------------------------
    torch.save(Model,"/root/Desktop/Net/Sleep_Net_2.pt")
